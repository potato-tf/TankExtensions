...
function TankExtPacked::PathMaker(hPlayer, sPathName = null)
{
	local Delete = []
	...
	if(sPathName)
	{
		...
		if(hPath)
		{
			...
			for(local i = 1; i < 256; i++)
			{
				...
				if(!hPreviousPath || TankExtPacked.GetNextPath(hPreviousPath) != hPath)
				{
					hStartPath = hPath
					break
				}
			...
			if(hStartPath)
			{
				...
				for(local i = 1; i < 256; i++)
				{
					local hNextPath = TankExtPacked.GetNextPath(hPath)
					if(hNextPath)
					{
	...
	TankExtPacked.SetDestroyCallback(hPathMaker, function()
	{
	...
	local function PathMakerThink()
	{
		...
		if(hNearestPathTrack)
		{
			...
			local hPathTrackNext = TankExtPacked.GetNextPath(hNearestPathTrack)
					...
					SpawnParticles = function()
					{
						foreach(Table in ParticleSpawns)
						{
							...
							if(iSpawnedParticles == 65)
							{
								...
								TankExtPacked.DelayFunction(null, null, 0.001, @() SpawnParticles())
								return
							}
	...
	TankExtPacked.AddThinkToEnt(hPathMaker, "PathMakerThink")
}