...
TankExtPacked.PrecacheSound(FRONTLINECART_ROCKET_SOUND)
TankExtPacked.PrecacheSound(FRONTLINECART_GRENADE_SOUND)
TankExtPacked.PrecacheSound(FRONTLINECART_JAR_SOUND)

TankExtPacked.NewTankType("frontlinecart*", {
	...
	function OnSpawn()
	{
		...
		TankExtPacked.SetParentArray([hModel, hFakeBomb], self)
		TankExtPacked.SetParentArray([hWeapon], hModel, "fire")
		TankExtPacked.DispatchParticleEffectOn(hModel, bBlueTeam ? "cart_flashinglight" : "cart_flashinglight_red", "light")
		...
		TankExtPacked.SetParentArray([hDisp], hModel, "light")
		...
		if(iProjectileType == PROJECTILE_JARATE)
		{
			...
			TankExtPacked.SetParentArray([hProp], hModel, "light")
		}
		...
		local Shoot = function(bCloseRange)
		{
			...
			local ApplyToProjectile = function(hEnt)
			{
				...
				if(iType == PROJECTILE_GAS)
				{
					...
					TankExtPacked.SetDestroyCallback(hEnt, function()
					{
				...
				TankExtPacked.AddThinkToEnt(hEnt, "FrontlineProjectileThink")
			}
		...
		function Think()
		{
			...
			{
				if(!hPlayer.IsAlive() || TankExtPacked.IsPlayerStealthedOrDisguised(hPlayer) || hPlayer.GetTeam() == iTeamNum) continue
				local vecDist = hPlayer.GetOrigin() - vecOrigin
				flAngle = (TankExtPacked.VectorAngles(vecDist) - angRotation).y
