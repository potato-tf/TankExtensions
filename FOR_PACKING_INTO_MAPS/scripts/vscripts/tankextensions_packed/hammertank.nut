...
::HammerTankEvents <- {
	...
	OnScriptHook_OnTakeDamage = function(params)
	{
		...
		{
			local HammerScope = TankExtPacked.GetMultiScopeTable(hAttacker.GetScriptScope(), "hammertank")
			if(HammerScope)
			{
...
TankExtPacked.PrecacheSound(")misc/halloween/strongman_fast_swing_01.wav")
TankExtPacked.PrecacheSound(")misc/halloween/strongman_fast_whoosh_01.wav")
TankExtPacked.PrecacheSound(")misc/halloween/strongman_fast_impact_01.wav")
TankExtPacked.PrecacheSound(")ambient/explosions/explode_1.wav")
TankExtPacked.PrecacheSound(")doors/vent_open2.wav")

TankExtPacked.NewTankType("hammertank", {
	function OnSpawn()
	{
		local hHammer = TankExtPacked.SpawnEntityFromTableFast("prop_dynamic", {
		...
		TankExtPacked.SetParentArray([hHammer], self)
			...
			if(!bSmashing)
				for(local hPlayer; hPlayer = FindByClassnameWithin(hPlayer, "player", vecSmash, 256);)
					if(hPlayer.IsAlive() && hPlayer.GetTeam() != iTeamNum)
					{
						...
						TankExtPacked.DelayFunction(self, this, 0.5, function() { EmitSoundEx({ sound_name = ")misc/halloween/strongman_fast_swing_01.wav", pitch = 70, sound_level = 100, entity = self, filter_type = RECIPIENT_FILTER_GLOBAL }) })
						TankExtPacked.DelayFunction(self, this, 1.0, function() { EmitSoundEx({ sound_name = ")misc/halloween/strongman_fast_whoosh_01.wav", sound_level = 100, entity = self, filter_type = RECIPIENT_FILTER_GLOBAL, }) })
						TankExtPacked.DelayFunction(self, this, 1.3, SmashImpact)
						TankExtPacked.DelayFunction(self, this, 3.4, function() { EmitSoundEx({ sound_name = ")doors/vent_open2.wav", pitch = 70, sound_level = 100, entity = self, filter_type = RECIPIENT_FILTER_GLOBAL }) })
						TankExtPacked.DelayFunction(self, this, 3.4 + HAMMERTANK_COOLDOWN, function() { bSmashing = false })
						break
					}
		}
	}
