...
TankExtPacked.PrecacheSound(")player/fall_damage_indicator.wav")
TankExtPacked.PrecacheSound(")weapons/rocket_pack_boosters_fire.wav")
TankExtPacked.PrecacheSound(")weapons/rocket_pack_boosters_charge.wav")

TankExtPacked.NewTankType("jumptank", {
	function OnSpawn()
	{
		local hTracks = []
		...
		TankExtPacked.SetParentArray([hParticle], self)
		...
		local function Jump()
		{
			...
			if(Trace.fraction >= 0.4)
			{
				...
				TankExtPacked.DelayFunction(self, JumpScope, 0.75, function()
				{
					local vecOrigin = self.GetOrigin()
					bJumping        = true
		...
		function Think()
		{
			...
			if(bJumping)
			{
				...
				if(!Trace.startsolid && Trace.hit)
					if(TankExtPacked.NormalizeAngle(TankExtPacked.VectorAngles(vecNormal).x) > -45)
					{
				...
				if(bJumping)
				{
					...
					foreach(hPlayer in ValidPlayers)
						if(hPlayer.IsValid() && hPlayer.IsAlive())
						{
							...
							if(TankExtPacked.IntersectionBoxBox(vecCheck, vecMins, vecMaxs, vecPlayer, hPlayer.GetPlayerMins(), hPlayer.GetPlayerMaxs()))
		...
		{
			bDeploying = true
			if(JUMPTANK_USE_SPECIAL_DEPLOY) TankExtPacked.DelayFunction(self, this, 3.5, Jump)
			else if(bJumping)
			{
				vecFakeVelocity.x = 0
				vecFakeVelocity.y = 0
