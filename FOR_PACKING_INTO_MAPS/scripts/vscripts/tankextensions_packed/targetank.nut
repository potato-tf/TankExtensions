...
TankExtPacked.PrecacheSound(TARGETANK_SND_WARNING)
TankExtPacked.PrecacheSound(TARGETANK_SND_IMPACT)
TankExtPacked.PrecacheSound(TARGETANK_SND_CHARGE)

TankExtPacked.NewTankType("targetank", {
	function OnSpawn()
	{
		local hTargeModel = TankExtPacked.SpawnEntityFromTableFast("prop_dynamic", {
		...
		function Think()
		{
			...
			if(iState == 0 && bCanDoAction)
			{
				...
				sSound()
				sSound()
				TankExtPacked.DelayFunction(self, this, 1, sSound)
				TankExtPacked.DelayFunction(self, this, 1, sSound)
			}
			...
			local Color = function(bool)
			{
				...
				TankExtPacked.SetTankColor(self, format("%i %i %i", vecColorCombined.x, vecColorCombined.y, vecColorCombined.z))
			}
...
TankExtPacked.NewTankType("targetank_color", {
	...
	function OnSpawn()
	{
		TankExtPacked.TankScripts.targetank.OnSpawn.call(this)
