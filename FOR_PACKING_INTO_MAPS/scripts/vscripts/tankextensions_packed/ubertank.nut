...
TankExtPacked.PrecacheSound(UBERTANK_SND_UBER)
...
::UberTankEvents <- {
	...
	OnScriptHook_OnTakeDamage = function(params)
	{
		...
		{
			local UberScope = TankExtPacked.GetMultiScopeTable(hVictim.GetScriptScope(), "ubertank")
...
TankExtPacked.NewTankType("ubertank*", {
	function OnSpawn()
	{
		...
		if(flTimeStart >= 0) TankExtPacked.DelayFunction(self, this, flTimeStart, @() ToggleUber())
		...
		function ToggleUber()
		{
			if(!bUberFizzle)
				if(!bUbered)
				{
					if(flDuration >= 0) TankExtPacked.DelayFunction(self, this, flDuration, @() ToggleUber())
					bUbered    = true
					EmitSoundEx({
					...
					TankExtPacked.SetTankModel(self, {
						Tank       = UBERTANK_MODEL
		...
		function Think()
		{
			if(bUbered)
			{
				...
				if(sModel != UBERTANK_MODEL)
				{
					...
					TankExtPacked.SetTankModel(self, UBERTANK_MODEL)
				}
			}
			if(bUberFizzle)
			{
