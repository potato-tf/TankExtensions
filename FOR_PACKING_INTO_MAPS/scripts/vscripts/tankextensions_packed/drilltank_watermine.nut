...
TankExtPacked.PrecacheSound(DRILLTANK_WATERMINE_SOUND_SPIN1)
TankExtPacked.PrecacheSound(DRILLTANK_WATERMINE_SOUND_SPIN2)
...
TankExtPacked.PrecacheSound(DRILLTANK_WATERMINE_SND_UBER)
TankExtPacked.PrecacheSound(DRILLTANK_WATERMINE_SND_UBER_OFF)
...
::DrillTankWatermineEvents <- {
	function OnScriptHook_OnTakeDamage(params)
	{
		...
		if(hVictim && hAttacker)
		{
			...
			{
				local DrillScope = TankExtPacked.GetMultiScopeTable(hAttacker.GetScriptScope(), "drilltank_watermine")
			...
			{
				local DrillTankWatermineScope = TankExtPacked.GetMultiScopeTable(hVictim.GetScriptScope(), "drilltank_watermine")
...
TankExtPacked.NewTankType("drilltank_watermine", {
	DisableBomb = 1
	Model = {
	...
	function OnSpawn()
	{
		...
		TankExtPacked.SetParentArray([hDrillHurt], self)
		...
		function OnStartDeploy()
		{
			...
			TankExtPacked.DelayFunction(self, this, 2 function() {
			...
			TankExtPacked.DelayFunction(self, this, 8, function() {
				EmitSoundEx({
					sound_name  = "physics/metal/canister_scrape_smooth_loop1.wav"
					entity      = self
					filter_type = RECIPIENT_FILTER_GLOBAL
