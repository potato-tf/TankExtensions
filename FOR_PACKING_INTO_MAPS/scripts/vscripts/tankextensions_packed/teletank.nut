local TELETANK_VALUES_TABLE = {
	...
	TELETANK_DISPENSER_HEALING = false
	TELETANK_DISPENSER_MINS    = Vector(-512, -512, -128)
	TELETANK_DISPENSER_MAXS    = Vector(512, 512, 384)
}
	...
	Bot_TeleTank = function(hPlayer)
	{
		...
		if(hPlayer.IsBotOfType(TF_BOT_TYPE) && hPlayer.HasBotTag("bot_teletank"))
			for(local hTank; hTank = FindByClassname(hTank, "tank_boss");)
			{
				...
				local TeleScope = TankExtPacked.GetMultiScopeTable(hTank.GetScriptScope(), "teletank")
...
TankExtPacked.NewTankType("teletank", {
	Model = {
	...
	function OnSpawn()
	{
		...
		if(TELETANK_DISPENSER_HEALING)
		{
			...
			hTouch.SetSize(TELETANK_DISPENSER_MINS, TELETANK_DISPENSER_MAXS)
			...
			hTouchReal.SetSize(TELETANK_DISPENSER_MINS, TELETANK_DISPENSER_MAXS)
