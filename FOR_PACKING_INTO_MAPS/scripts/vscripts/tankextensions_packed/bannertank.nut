...
TankExtPacked.PrecacheSound(BANNERTANK_BUFF_SOUND_RED)
TankExtPacked.PrecacheSound(BANNERTANK_BUFF_SOUND_BLUE)
TankExtPacked.PrecacheSound(BANNERTANK_CONCH_SOUND_RED)
TankExtPacked.PrecacheSound(BANNERTANK_CONCH_SOUND_BLUE)
TankExtPacked.PrecacheSound(BANNERTANK_BACKUP_SOUND_RED)
TankExtPacked.PrecacheSound(BANNERTANK_BACKUP_SOUND_BLUE)
TankExtPacked.PrecacheSound(BANNERTANK_SOUND_FLAG)
	...
	function OnScriptHook_OnTakeDamage(params)
	{
		...
		if(hAttacker && hVictim)
		{
			local BannerCheck = function(hEnt, iBannerType)
			{
				...
				{
					local BannerScope = TankExtPacked.GetMultiScopeTable(hEnt.GetScriptScope(), "bannertank")
...
TankExtPacked.NewTankType("bannertank*", {
	function OnSpawn()
	{
		...
		TankExtPacked.SetParentArray([hPackBuff, hBannerBuff, hPackConch, hBannerConch, hPackBackup, hBannerBackup], self)
		...
		function SetBanner(iAdd, iRemove)
		{
			...
			if(iAdd)
			{
				...
				{
					bAddedBanner = true
					TankExtPacked.DelayFunction(self, this, 0.22, @() Sound(bBlueTeam ? sSoundBlue : sSoundRed) )
					TankExtPacked.DelayFunction(self, this, BANNERTANK_ACTIVATE_DELAY, @() hBanner.AcceptInput("Enable", null, null, null) )
				}
				...
				if(!(iActiveBanners & BANNER_CONCH) && iAdd & BANNER_CONCH)
				{
					...
					TankExtPacked.DelayFunction(self, this, BANNERTANK_ACTIVATE_DELAY, function() { if(!bNoSelfEffect) SetPropFloat(self, "m_speed", GetPropFloat(self, "m_speed") * BANNERTANK_CONCH_SELF_SPEED_MULT) })
				}
				...
				if(bAddedBanner) TankExtPacked.DelayFunction(self, this, BANNERTANK_ACTIVATE_DELAY, function()
				{
			...
			if(iRemove)
			{
				if(iActiveBanners & BANNER_BUFF && iRemove & BANNER_BUFF)
				{
					...
					TankExtPacked.DelayFunction(self, this, 0.15, function() { hPackBuff.AcceptInput("Disable", null, null, null) })
				}
				if(iActiveBanners & BANNER_CONCH && iRemove & BANNER_CONCH)
				{
					...
					TankExtPacked.DelayFunction(self, this, 0.15, function() { hPackConch.AcceptInput("Disable", null, null, null) })
				...
				if(iActiveBanners & BANNER_BACKUP && iRemove & BANNER_BACKUP)
				{
					...
					TankExtPacked.DelayFunction(self, this, 0.15, function() { hPackBackup.AcceptInput("Disable", null, null, null) })
				}
