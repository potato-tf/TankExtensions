...
TankExtPacked.NewTankType("gatetank*", {
	Model = {
	...
	function OnDeath()
	{
		...
		if(bCapturing && hObjectiveResource.IsValid() && GetPropIntArray(hObjectiveResource, "m_iNumTeamMembers", iPointArray) == 0)
		{
			...
			SetPropIntArray(hObjectiveResource, "m_iTeamInZone", 0, iPointIndex)
			TankExtPacked.DelayFunction(hObjectiveResource, null, (1 - flCapturePercent) * GetPropFloatArray(hObjectiveResource, "m_flTeamCapTime", iPointArray), function()
			{
