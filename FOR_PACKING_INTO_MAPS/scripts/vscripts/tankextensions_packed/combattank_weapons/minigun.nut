...
TankExtPacked.PrecacheSound(COMBATTANK_MINIGUN_SND_SPINUP)
TankExtPacked.PrecacheSound(COMBATTANK_MINIGUN_SND_SPINNING)
TankExtPacked.PrecacheSound(COMBATTANK_MINIGUN_SND_FIRE)
TankExtPacked.PrecacheSound(COMBATTANK_MINIGUN_SND_SPINDOWN)

TankExtPacked.CombatTankWeapons["minigun"] <- {
	...
	function OnSpawn()
	{
		...
		TankExtPacked.SetParentArray([hParticleMuzzle1], self, "barrel_1")
		TankExtPacked.SetParentArray([hParticleMuzzle2], self, "barrel_2")
		TankExtPacked.SetParentArray([hParticleCasing1], self, "case_eject_1")
		TankExtPacked.SetParentArray([hParticleCasing2], self, "case_eject_2")
				...
				if(flTime >= flTimeIdle || iState > 1)
				{
					...
					if(bEnemyInCone)
					{
						...
						if(flTime >= flNextAttack)
						{
							...
							local hParticleTracer = TankExtPacked.SpawnEntityFromTableSafe("info_particle_system", {
								effect_name = COMBATTANK_MINIGUN_PARTICLE_TRACER
								start_active = 1
							})
							TankExtPacked.SetParentArray([hParticleTracer], self, iBarrel ? "barrel_1" : "barrel_2")
							local hTracerTarget = TankExtPacked.SpawnEntityFromTableSafe("info_target", { origin = Trace.endpos, spawnflags = 0x01 })
