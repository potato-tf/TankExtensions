...
TankExtPacked.PrecacheSound(COMBATTANK_CANNON_SND_FIRE)
TankExtPacked.PrecacheSound(COMBATTANK_CANNON_SND_FIRE2)
TankExtPacked.PrecacheSound(COMBATTANK_CANNON_SND_ROCKET_EXPLOSION)

TankExtPacked.CombatTankWeapons["cannon"] <- {
	TurretReplacement = 1
	function SpawnModel()
	{
	...
	function OnSpawn()
	{
		...
		function CombatTankWeaponThink()
		{
			...
			self.SetPoseParameter(iPosePitch, TankExtPacked.Clamp(hTank_scope.angCurrent.x, -12, 30))
			...
			if(bCanAttack && bEnemyInRadius)
			{
				...
				for(local hRocket; hRocket = FindByClassnameWithin(hRocket, "tf_projectile_rocket", hWeapon.GetOrigin(), 64);)
				{
					...
					TankExtPacked.SetDestroyCallback(hRocket, function()
					{
						local Trace = {
							start = vecOrigin
					...
					{
						TankExtPacked.DispatchParticleEffectOn(hRocket, null)
						TankExtPacked.DispatchParticleEffectOn(hRocket, COMBATTANK_CANNON_PARTICLE_TRAIL, "trail")
					}
				}
			}
