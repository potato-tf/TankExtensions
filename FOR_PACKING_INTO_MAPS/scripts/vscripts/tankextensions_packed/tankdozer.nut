...
::TankDozerEvents <- {
	OnScriptHook_OnTakeDamage = function(params)
	{
		...
		{
			local DozerScope = TankExtPacked.GetMultiScopeTable(hVictim.GetScriptScope(), "tankdozer")
			if(DozerScope)
			{
				...
				TraceRecursive = function()
				{
					...
					if("enthit" in Trace)
					{
						...
						if(!(hEntHit in CrateCollision))
						{
							...
							else if(sClassname == "worldspawn" && Trace.end != vecCenter)
							{
								Trace.end = vecCenter
								TraceRecursive()
							}
...
TankExtPacked.NewTankType("tankdozer*", {
	function OnSpawn()
	{
		...
		TankExtPacked.SetParentArray(ModelArray, self)
		...
		if(sTankName.find("_nosentry") == null)
		{
			...
			TankExtPacked.SetParentArray([hSentry], self)
		}
