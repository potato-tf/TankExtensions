...
TankExtPacked.PrecacheSound(TANKDOZER_SND_BREAKABLE_HURT)
TankExtPacked.PrecacheSound(TANKDOZER_SND_SENTRY_SAPPED)

TankExtPacked.NewTankType("tankdozer*", {
	function OnSpawn()
	{
		local arrayModels = []
		local SpawnBreakable = @(vecOrigin, sModel) TankExtPacked.SpawnEntityFromTableFast("prop_dynamic", { origin = vecOrigin, model = sModel, solid = SOLID_VPHYSICS, "OnTakeDamage" : "!selfRunScriptCodeEmitSoundOn(TANKDOZER_SND_BREAKABLE_HURT,self)-1-1" })
		...
		arrayModels.append(TankExtPacked.SpawnEntityFromTableFast("prop_dynamic", { model = TANKDOZER_MODEL, solid = SOLID_VPHYSICS }))
		...
		if(bSentry)
		{
			...
			TankExtPacked.AddThinkToEnt(hSentry, "SentryThink")
		}
		TankExtPacked.SetParentArray(arrayModels, self)
	}
})