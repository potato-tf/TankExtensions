// Last Updated : 3:05PM PST July 12 2026
...
::TankExtPacked <- {
	...
	function CreateLoopPaths(PathTable)
	{
		foreach(sPathName, OriginArray in PathTable)
		{
			...
			TankExtPacked.SetPathConnection(hPath1, hPath2)
			TankExtPacked.SetPathConnection(hPath2, hPath3)

			hPath2.ValidateScriptScope()
			local hPath_scope = hPath2.GetScriptScope()
			IncludeScript("tankextensions_packed/misc/loopingpath_think", hPath_scope)
	...
	function NewTankType(sName, Table)
	{
		...
		if(sName[sName.len() - 1] == '*')
		{
			...
			TankExtPacked.TankScriptsWild[sName] <- Table
		}
		else
			TankExtPacked.TankScripts[sName] <- Table
	}
	...
	function ExtraTankKeyValues(hTank, hPath, TankTable)
	{
		...
		if(Check("Model"))
		{
			...
			TankExtPacked.SetTankModel(hTank, {
			...
			hTank_scope.ModelThink <- function()
			{
				...
				if(sModel != sModelLast)
				{
					if(bModel)
					{
						...
						TankExtPacked.SetTankModel(hTank, { Tank = sNewModel })
						sModel = sNewModel
					}
				}
				sModelLast = sModel
			}
			TankExtPacked.AddThinkToEnt(hTank, "ModelThink")
		}
		...
		if(Check("Color"))
			TankExtPacked.SetTankColor(hTank, TankTable.Color), Add("Color")
		...
		if(Check("NoScreenShake") && TankTable.NoScreenShake)
		{
			...
			TankExtPacked.AddThinkToEnt(hTank, "NoScreenShakeThink")
		}
		...
		if(Check("EngineLoopSound"))
		{
			...
			TankExtPacked.PrecacheSound(sSound)
			...
			TankExtPacked.AddThinkToEnt(hTank, "EngineLoopSoundThink")
		...
		if(Check("PingSound"))
		{
			...
			TankExtPacked.AddThinkToEnt(hTank, "PingSoundThink")
		}
		...
		if(Check("UseCustomLocomotion") && TankTable.UseCustomLocomotion)
		{
			...
			angRotation.x = TankExtPacked.NormalizeAngle(angRotation.x)
			angRotation.y = TankExtPacked.NormalizeAngle(angRotation.y)
			...
			hTank_scope.CustomLocomotionThink <- function()
			{
				...
				if(hGoal)
				{
					...
					if(hGoal)
					{
						...
						local flPitchGoal = TankExtPacked.NormalizeAngle(TankExtPacked.VectorAngles(vecForward).x)
	...
	function ApplyTankTableByName(hTank, hPath, sTankName)
	{
		...
		if(TankTable)
		{
			...
			if(!("MultiScope" in hTank_scope))
			{
				...
				TankExtPacked.AddThinkToEnt(hTank, "MultiScopeThink")
			}
	...
	function AddThinkToEnt(hEntity, sFunction)
	{
		...
		if(hEntity.GetClassname() == "tank_boss")
		{
			...
			hTank.ValidateScriptScope()
			local hTank_scope = hTank.GetScriptScope()
			if(!("TankExtPackedTankThink" in hTank_scope))
			{
				...
				AddThink(hTank, "TankExtPackedTankThink")
			}
...
for(local i = 1; i <= MAX_CLIENTS; i++)
{
	...
	if(hPlayer)
		TankExtPacked.PlayerArray.append(hPlayer)
}
...
local function TankExtPackedThink()
{
...
hThinkEnt_scope.TankExtPackedThink <- TankExtPackedThink
TankExtPacked.AddThinkToEnt(hThinkEnt, "TankExtPackedThink")
