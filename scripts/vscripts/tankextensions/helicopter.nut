	...
	function OnSpawn()
	{
		...
		function Think()
		{
			...
			if(bLaunched)
			{
				...
				local hTarget
				local vecTarget
				foreach(i, sClassname in [ "player", "obj_sentrygun", "obj_dispenser", "obj_teleporter", "tank_boss", "merasmus", "headless_hatman", "eyeball_boss", "tf_zombie" ])
				{
					...
					for(local hEnt, flDist = HELICOPTER_MAX_RANGE; hEnt = FindByClassnameWithin(hEnt, sClassname, vecOrigin, flDist);)
					{
						...
						if(!hEnt.IsAlive() || hEnt.GetTeam() == iTeamNum || hEnt.GetFlags() & FL_NOTARGET || (TankExt.IsPlayerStealthedOrDisguised(hEnt) && hEnt.GetDisguiseTeam() == iTeamNum))
							continue
